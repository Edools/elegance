(function () {
  ...
  */
  ...
  var getPaymentMethod = function (enrollmentId, courseId, apiKey) {
    ...
    console.warn('executed: lessonListCards.getPaymentMethod');
  ...
  var getLessonsInfo = function (enrollmentId, courseId, apiKey) {
    ...
    console.warn('executed: lessonListCards.getLessonInfo');
  ...
  app.lessonListCards= {
    ...
    init: function () {
      ...
      self.courseContent = self.courseTree().data('course-content2');
      ...
      if (self.courseTreeExists && !self.isActive) {
        ...
        $(document).on('lesson-completed', function (event, lessonProgress) {
          ...
          app.lessonListCards.requirementsExists(lessonProgress.data, function ($item, content_id) {
            if ($item && content_id) {
              app.lessonListCards.checkLessonCompleted(enrollmentId, content_id, function (completed) {
                if (completed) {
    ...
    // Props
    courseTree: function () {
      return $('.course-content2 #js-course-tree-ajax3');
    },

    lazyLoadButtons: function () {
      return $('.course-content2 #js-course-content-more-link, .course-content2 #js-course-content-prev-link');
    },
    ...
    checkLessonCompleted: function (enrollmentId, id, cb) {
      var self = this;
      var completed = false;
      var apiKey = $('.course-content2 #js-course-tree-ajax3').data('api-key');
    ...
    bindClicks: function () {
      ...
      self.lessons().find('a').on('click', function (e) {
        e.preventDefault();
        app.lessonListCards.changeLesson(null, $(this).parents('.js-content'));
      });
      ...
      return $.ajax({
        ...
        success: function (res) {
          ...
          var $courseContents = _.map(courseContents, function (content) {
              ...
              var html = '<li class="list-group-item content-lesson list-group-item lesson module-item ' + active + (!available || (self.enrollment && self.checkTrialByType('content', content.id)) ? ' blocked' : '') + '" ' +
                ...
                '<div class="center"><i class="' + lessonIcon + '"></i></div>' +

                '<div class="teste"><div class="center">' +
    ...
    checkNextButtonUnlocked: function () {
      ...
      app.lessonListCards.requirementsExists(lessonProgress, function ($item, content_id) {
        if ($item && content_id) {
          if (!content_id || !lessonProgress.hasOwnProperty('enrollment_id')) {
            return;
          }

          app.lessonListCards.checkLessonCompleted(lessonProgress.enrollment_id, content_id, function (completed) {
            if (completed) {
              $('.btn-next-lesson').removeClass('disabled');
              $item.removeClass('blocked');
              $item.find('.right > .icon-lock').remove();
    ...
    handleLessons: function () {
      $(document).on('page:load page:restore', function (e) {
        ...
        app.lessonListCards.lessons().each(function (i, el) {
          var $el = $(el);
    ...
    toggleSidebar: function () {
      app.lessonListCards.toggleButton().toggleClass('active');
      app.lessonListCards.lessonListPanel().toggleClass('active');
    }
  };
